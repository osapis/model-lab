...
function RunDetail({ run, busy, mutate }: { run: Run; busy: string; mutate: Mutate }) {
  ...
  return <div className="admin-run-detail">
    ...
    {run.cleanupError && <div className="admin-alert admin-cleanup-alert" role="status"><Clock3 size={16} /><span>云端作品尚未清理成功，历史记录暂时保留，系统将每分钟重试。{run.cleanupError}</span></div>}
    <RetryInfo run={run} />
    <div className="admin-run-meta"><span>API 接口 <strong>{run.providerName}</strong></span><span>模型 ID <code>{run.modelSlug}</code></span><span>输入 / 输出 tokens <strong>{run.inputTokens ?? '—'} / {run.outputTokens ?? '—'}</strong></span><span>协议 <strong>{run.parameters.protocol ?? '—'}</strong></span><span>推理强度 <strong>{run.parameters.reasoningEffort || '默认'}</strong></span><span>单次超时 <strong>{run.requestTimeoutSeconds ? `${run.requestTimeoutSeconds} 秒` : '未记录'}</strong></span><span>作品存储 <strong>{run.artifactStorage === 'cloudflare' ? 'Cloudflare 云端' : run.artifactStorage === 's3' ? 'R2 / S3 云端' : '内存缓存'}</strong></span>{run.artifactExpiresAt && <span>保留至 <strong>{dateLabel(run.artifactExpiresAt)}</strong></span>}</div>
    ...
    <div className="admin-output-box"><div className="admin-output-toolbar"><div className="admin-output-tabs">{runTabs.map(({ id, label }) => <button type="button" key={id} className={outputTab === id ? 'active' : ''} onClick={() => setOutputTab(id)}>{id === 'html' && <Code2 size={13} />}{label}</button>)}</div>
      ...
      {artifactMissing ? <div className="admin-output-empty admin-cache-expired"><Database size={25} /><strong>{run.artifactStorage === 'cloudflare' ? '作品正文暂不可用，历史记录仍保留' : '作品缓存已失效，历史记录仍保留'}</strong><p>{run.artifactStorage === 'cloudflare' ? '本次测试的接口、状态、耗时和参数仍可查看。可在结果存储设置中验证云端读写状态。' : '本次测试的接口、状态、耗时和参数仍可查看。需要长期回看完整作品，请为后续测试配置 Cloudflare R2 / S3 存储。'}</p></div> : outputTab === 'preview' && run.html ? <div className="admin-preview"><Preview html={run.html} title={run.promptTitle} /></div> : selectedText ? <pre className={`admin-raw-output ${outputTab === 'html' ? 'code' : ''}`}>{selectedText}</pre> : <div className="admin-output-empty">{active ? <><Spinner /><span>{run.status === 'queued' ? '任务正在排队，结果将自动更新。' : '模型正在生成，完成后会显示完整输出。'}</span></> : <span>{run.status === 'failed' ? '本次测试未获得有效输出，可检查错误后重试。' : '此记录没有返回内容。'}</span>}</div>}
    </div>
